  ...
  React.useEffect(() => {
    const timeout = setTimeout(() => {
      ...
      scrollRef.current.scrollTo({
        x: screenWidth * (newIndex > 3 ? 0 : newIndex),
      });
    }, 5000);
    return () => {
    ...
    <View style={styles.container}>
          ...
          <View key={index}>
            ...
            <View style={styles.hashtagContainer}>
              <View style={styles.hashtagBox}>
                <Text style={styles.hashtagText}>{data.hashtag.first}</Text>
              </View>
              ...
              >
                <Text style={styles.hashtagText}>{data.hashtag.second}</Text>
              </View>
            </View>

            <Text style={styles.sliderTitle}>{data.title}</Text>
          </View>
        ))}
      </ScrollView>
      ...
      <View style={styles.sliderDots}>
        {dotList.map((dot, index) => (
          <View
            key={index}
            style={imgIndex === dot.id ? styles.dotActive : styles.dot}
          ></View>
        ))}
      </View>
    </View>
